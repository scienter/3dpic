...
void MPI_TransferF_DSX_YminusC(Domain *D,float ***f1,float ***f2,float ***f3)
{
    int i,j,k,numberData,start,end,nx,nySub,nzSub,ibegin,ibottom;
    int istart,iend,jstart,jend,kstart,kend;
    int myrank, nTasks,rank; 
    ...
    if(rank%2==0 && rank!=D->M-1)
    {
      MPI_Recv(D->minusYC,numberData, MPI_FLOAT, myrank+1, myrank+1, MPI_COMM_WORLD,&status);  
      start=0; 
      for(j=0; j<1; j++)	// 3 layers in y direction
        for(k=0; k<nzSub+5; k++)
        {
       ...
       MPI_Send(D->minusYC,numberData, MPI_FLOAT, myrank-1, myrank, MPI_COMM_WORLD);             
    MPI_Barrier(MPI_COMM_WORLD);
                          ...
                          ,float ***f4,float ***f5,float ***f6)
{
    ...
    share=3;
    numberData=share*6*(nx+5)*(nzSub+5); 
    rank=myrank%D->M;   
...
void MPI_TransferF_DSX_YplusC(Domain *D,float ***f1,float ***f2,float ***f3)
{
    int i,j,k,numberData,start,end,nx,nySub,nzSub,ibegin,ibottom;
    int istart,iend,jstart,jend,kstart,kend;
    int myrank, nTasks,rank; 
    ...
    if(rank%2==1)
    {
       MPI_Recv(D->plusYC,numberData, MPI_FLOAT, myrank-1, myrank-1, MPI_COMM_WORLD,&status);  
       start=0;
       for(j=1; j<2; j++)
         for(k=0; k<nzSub+5; k++)
         {
       ...
       MPI_Send(D->plusYC,numberData, MPI_FLOAT, myrank+1, myrank, MPI_COMM_WORLD);             
    MPI_Barrier(MPI_COMM_WORLD);
                      ...
                      ,float ***f4,float ***f5,float ***f6)
{
    ...
    share=3;
    numberData=(share-1)*6*(nx+5)*(nzSub+5); 
    rank=myrank%D->M;   
